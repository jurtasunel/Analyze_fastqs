...
echo Converting to sam to bam file...
...
Rscript $Rscripts/plot_depth.R `pwd`/${rawfastq}_depth.txt

# Move the plot and csv files generated by the Rscript.
mv -t ${rawfastq}_results ${rawfastq}_not_aligned.txt ${rawfastq}_depth.txt ${rawfastq}_depth.pdf ${rawfastq}_low_depth_positions.csv
